...
def generate_password():
    ...
    if length < 15 or length > 20:
        messagebox.showerror("Error", "Password length must be between 15 and 20!")
        return

    # Build secure character pool
    character_pool = ""
    if use_uppercase:
...
# Length selection (15–20)
tk.Label(root, text="Password Length (15–20):", font=("Arial", 10, "bold")).pack()
...
tk.Checkbutton(root, text="Include Uppercase (A-Z)", variable=uppercase_var).pack(anchor='w')
tk.Checkbutton(root, text="Include Lowercase (a-z)", variable=lowercase_var).pack(anchor='w')
...
# Generate button
generate_button = tk.Button(root, text="Generate Secure Password", command=generate_password, bg="#4CAF50", fg="white")
